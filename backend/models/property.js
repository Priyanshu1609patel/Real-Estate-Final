...
const Property = {
  ...
  getById: (id, callback) => {
    pool.query('SELECT * FROM properties WHERE id = $1', [id], (err, result) => {
      if (err) return callback(err);
      callback(null, result.rows);
    });
  },
  ...
  update: (id, data, callback) => {
    ...
    pool.query(query, [...values, id], (err, result) => {
      if (err) return callback(err);
      callback(null, result.rows[0]);
    });
  },
  delete: (id, callback) => {
    pool.query('DELETE FROM properties WHERE id = $1 RETURNING *', [id], (err, result) => {
      if (err) return callback(err);
      callback(null, result.rows[0]);
    });
  }
